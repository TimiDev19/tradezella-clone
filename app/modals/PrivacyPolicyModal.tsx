...
type PrivacyPolicyModalProps = {
    setIsPrivacyPolicyModalOpen: (open: boolean) => void;
};

const PrivacyPolicyModal: React.FC<PrivacyPolicyModalProps> = ({ setIsPrivacyPolicyModalOpen }) => {
    return (
        <div
        ...
        >
            <div className=' h-[75vh] lg:h-[60vh] overflow-y-scroll w-full bg-white dark:bg-black rounded-t-3xl p-2'>
                <button onClick={() => setIsPrivacyPolicyModalOpen(false)} className=' mb-[20px] block cursor-pointer w-[100px] h-[4px] rounded-full bg-slate-200 dark:bg-white/30 mx-auto'></button>
                <h1 className=' font-extrabold text-3xl text-center mb-[10px] text-black dark:text-white'>Privacy Policy</h1>
                <p className=' text-black dark:text-white/30 text-center mb-[10px]'>Read through our privacy policy</p>
                <p className=' w-[98%] lg:w-[70%] mx-auto text-black dark:text-white/30 text-[13px] lg:text-sm'>
